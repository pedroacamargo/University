...
\pagebreak
	...
	\chapter{Contextualização}
        ...
        \subsection{Infraestrutura e Componentes}
    ...
    \chapter{Análise de Riscos}
        ...
        Algumas causas possíveis para uma falha no servidor incluem:
        \begin{itemize}
        \item Falta de energia elétrica no local de hospedagem;
        \item Ciberataques direcionados ao servidor web;
        \item Erros humanos durante manutenção ou atualização do sistema.
        \end{itemize}

        Para mitigar este risco, recomenda-se a implementação de mecanismos de \textbf{redundância}, \textbf{replicação} e \textbf{failover}, garantindo alta disponibilidade do serviço.

        \section{Gargalos de Desempenho}

        À medida que o número de utilizadores cresce, o servidor pode tornar-se um gargalo, resultando em tempos de resposta mais lentos ou até falhas.  
        Como a aplicação \textbf{AirTrail} utiliza o \textbf{SvelteKit} com \textbf{Server-Side Rendering (SSR)}, cada requisição ao servidor requer processamento adicional, o que aumenta a carga computacional.  
