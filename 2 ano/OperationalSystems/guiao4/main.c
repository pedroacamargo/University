#include <stdio.h>
#include <sys/wait.h>
#include <unistd.h>

int PaiParaFilho() {

    int fds[2];

    if (pipe(fds) == -1) {
        perror("pipe");
        return 1;
    }

    pid_t pid = fork();
    if (pid == -1) {
        perror("fork");
        return 1;
    }

    switch (pid) {
        case -1: {
            perror("Fork failed");
            break;
        }
        case 0: {
            close(fds[1]);
            int value_read;
            printf("Filho: | i'll read\n");
            int bytesRead = read(fds[0], &value_read, sizeof(int));
            printf("Filho: | read %d bytes, %d\n", bytesRead, value_read);
            close(fds[0]);
            printf("Filho: | finished\n");
            break;
        }
        default: {
            // pai
            close(fds[0]);
            int n = 300;
            sleep(3);
            printf("Pai: | i'll write %d\n", n);
            int bytesWritten = write(fds[1], &n, sizeof(int));
            printf("Pai: | wrote %d bytes\n", bytesWritten);
            close(fds[1]);
            int status;
            int wait_res = wait(&status);
            if (WIFEXITED(status)) {
                printf("Pai: | child exited with status %d\n", wait_res, WEXITSTATUS(status));
            } else {
                printf("Pai: | child exited abnormally\n");
            }
            break;
        }
    }    



    return 0;
}

int FilhoParaPai() {

    int fds[2];

    if (pipe(fds) == -1) {
        perror("pipe");
        return 1;
    }

    pid_t pid = fork();
    if (pid == -1) {
        perror("fork");
        return 1;
    }

    switch (pid) {
        case -1: {
            perror("Fork failed");
            break;
        }
        case 0: {
            close(fds[0]);
            for (int i = 0; i < 10000; i++) {
                int n = i+1;
                sleep(1);
                printf("Filho: | i'll write %d\n", n);
                int bytesWritten = write(fds[1], &n, sizeof(int));
                printf("Filho: | wrote %d bytes\n", bytesWritten);
            }
            close(fds[1]);
            break;
        }
        default: {
            // pai
            close(fds[1]);
            int value_read;
            int bytes_read = 1;
            while (bytes_read > 0) {
                printf("Pai: | i'll read\n");
                bytes_read = read(fds[0], &value_read, sizeof(int));
                printf("Pai: | read %d bytes with value %d\n", bytes_read, value_read);
            }
            close(fds[0]);
            printf("Pai: | finished\n");

            int status;
            int wait_res = wait(&status);
            if (WIFEXITED(status)) {
                printf("Pai: | child exited with status %d\n", wait_res, WEXITSTATUS(status));
            } else {
                printf("Pai: | child exited abnormally\n");
            }
            break;
        }
    }     // PRIMEIRO LEMOS DO PIPE E DEPOIS ESPERAMOS



    return 0;
}

int main() {
    return FilhoParaPai();
}