...
// 	Load the height map "terreno.jpg"
	...
	for (int i = 0; i < tw; i++) {
        for (int j = 0; j < th; j++) {
			...
			Point3D point1 = {
				i - tw * 0.5f + 1,
				imageData[i * tw + j + 1] / 2.0f, // height
				j - th * 0.5f
			};
			...
			Point3D point2 = {
				i - tw * 0.5f,
				imageData[i * tw + j] / 2.0f,
				j - th * 0.5f
			};
			p.push_back(point2);

        	// p.push_back(i - tw * 0.5 + 1);
        	// p.push_back(imageData[i * tw + j + 1] / 2);
        	// p.push_back(j - th * 0.5);

        	// p.push_back(i - tw * 0.5);
        	// p.push_back(imageData[i * tw + j] / 2);
        	// p.push_back(j - th * 0.5);
        }
    }
	...
	glBindBuffer(GL_ARRAY_BUFFER, vertices);
	glBufferData(GL_ARRAY_BUFFER, sizeof(Point3D) * p.size(), p.data(), GL_STATIC_DRAW);
	glEnableClientState(GL_VERTEX_ARRAY);
	...
	for (int i = 0; i < tw; i++) {
		for (int j = 0; j < th; j++) {
			...
			colors.push_back(brightness * 0.5f); // R
			colors.push_back(brightness); // G
			...
			colors.push_back(brightness2 * 0.5f); // R
